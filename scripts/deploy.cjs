...
const env = args.env || "test"; // Default a "production"
...
if (!["test", "production"].includes(env)) {
  console.error(
    "Errore: Il valore di --env deve essere 'test' o 'production'."
  );
  process.exit(1);
}
...
(async () => {
  try {
    ...
    // Build con l'ambiente specificato
    console.log(`Build con l'ambiente: ${env}`);
